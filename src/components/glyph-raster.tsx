  ...
  ({
    ...
    useVisibleTask$(
      ({ cleanup }: TaskCtx) => {
        ...
        const canvas = document.querySelector(`#${rasterId}`) as GlyphInitialFrameCanvas | null;
        if (!canvas) {
          return;
        }

        canvas.__glyphRuntimeActive = true;
        removeRuntimeMarker = (): void => {
          delete canvas.__glyphRuntimeActive;
        };
        ...
        const resize = (): void => {
          ...
          const grid = resolveGlyphGrid({
            ...
            maxCells:
              MAX_GLYPH_GRID_CELLS *
              (canvasAnchorMode === "document" ? DOCUMENT_ANCHOR_OVERSCAN : 1),
          });
